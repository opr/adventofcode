...
pub fn task() {
    ...
    let z = find_from_range( seeds, &mut parsed_maps );

    // let mut found = false;
    // //let mut location_check = 10283860usize;
    // let mut location_check = 17083861usize;
    // while ! found {
    //     let seed_found = reverse_lookup( &location_check, &parsed_maps );
    //     let matched_seed = seeds.iter().find( |(seed, range)| (seed..&(seed+range)).contains(&&seed_found) );
    //     if let Some( seed ) = matched_seed {
    //         println!("Found a match so i did");
    //         found = true;
    //         break;
    //     }
    //     location_check+=1;
    // }
    println!("Day 5 task 2 answer: {}", z);
...
fn find_from_range( mut ranges: Vec<(usize, usize)>,  maps: &mut Vec<Vec<LookupTable>> ) -> usize {
    ranges = ranges.iter().map( | r | (r.0,(r.0+r.1)-1) ).collect();
    for map in maps {
...
fn convert_ranges_with_map(range: &(usize, usize), map: &mut Vec<LookupTable> ) -> Vec<(usize,usize)> {
    ...
    for mapping in map {
        ...
        if min < source_end && max >= source_start {
            ...
            let new_range = ( destination_start + ( mapped_min - source_start ), destination_start + ( mapped_max - source_start) );
