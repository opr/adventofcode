...
fn parse_input( input: String ) -> ( Vec<Vec<usize>>, HashMap<usize,Vec<usize>> ) {
    ...
    let page_map = ordering_rules
        .iter()
        .fold( HashMap::<usize,Vec<usize>>::new(), | mut map, val | {
...
fn part1( input: String ) -> usize {
    ...
    for update in updates {
        ...
        for ( index, page ) in update.iter().enumerate() {
            ...
            let pages_before: Vec<&usize> = update.iter().take( index ).collect();
...
fn part2( input: String ) -> usize {
    ...
    for update in invalid_updates.clone().iter_mut() {
        ...
        for page in update {
            let must_come_before = page_map.get( page ).unwrap_or( &empty_set );
            let pages_before: Vec<&usize> = new_update.iter().collect();
